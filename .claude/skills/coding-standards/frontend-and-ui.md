...
Display prices via `formatPrice()` from `~/lib/utils`. Prices are stored in cents as integers (see [database.md](database.md)); `formatPrice` handles the "Free" case for `0`/`null`.
