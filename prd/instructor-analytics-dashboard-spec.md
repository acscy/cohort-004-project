...
Add an analytics view to the existing instructor dashboard (alongside the current per-student progress/quiz view in `instructor.$courseId.tsx`) showing, per instructor: revenue over time with a range toggle, a completion-rate KPI, an average-rating KPI, and — as a secondary, likely-deferred item — a lesson drop-off funnel. For instructors with more than one course, revenue is shown as a grid of small per-course charts rather than a single combined line, so one course's performance can't hide behind another's.
...
- **Per-course breakdown display:** small multiples (one compact chart per course, in a grid, sorted by whichever course most needs attention) — not a single summed line (which can mask a declining course behind a growing one) and not a single chart with all courses' lines overlaid (which stops being readable past a handful of courses).
